...
namespace HyCtbuEco.Services
{
    ...
    public class UserAuthorizeGuest : FilterAttribute, IAuthorizationFilter
    {

        public void OnAuthorization(AuthorizationContext filterContext)
        {
            //
            if (GuestUserInfo.GetGuestID() < 1)
            {
                //未认证
                var redirectUrl = "/Login/Guester?RedirectPath=" + filterContext.HttpContext.Request.Url;
