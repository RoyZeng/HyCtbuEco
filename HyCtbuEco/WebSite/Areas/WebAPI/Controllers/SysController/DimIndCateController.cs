...
{
  public class DimIndCateController : BaseController
   {
       
        #region gen code
		
		
		protected DimIndCateDAO DimIndCateDAO = new DimIndCateDAO();
		protected int totalCount = 0;
		
		 /// <summary>
		 /// GET api/DimIndCate/Get
		 ///返回所有的表格数据，json或xml格式
		 /// </summary>
        public JsonResult GetAll()
        {
			IList<TbDimIndCate> DimIndCateList =  DimIndCateDAO.GetAll();  
			return Json(DimIndCateList, JsonRequestBehavior.AllowGet);
           
        }

		/// <summary>
        /// GET api/DimIndCate/Get/id
		///返回指定id的数据
		/// </summary>
		/// <param name="id">id,主键</param>
        public JsonResult Get(int id)
        {
            TbDimIndCate item = DimIndCateDAO.GetByID(id);
            ...
            string orderBy, string condition)
        {
            ...
            IList<TbDimIndCate> userList= DimIndCateDAO.GetByPageDataBase(firstResult, pagesize, orderBy, condition, out totalCount);
            return Json(new
            {
        ...
        public int  Put(int id, TbDimIndCate item)
        {
		 ...
		 /// <summary>
        /// DELETE api/DimIndCate/Delete/5
        /// </summary>
        public int Delete(int id)
        {
			//tree
            IList<TbDimIndCate> chidrens = DimIndCateDAO.findByIParentID(id);
            if (chidrens.Count > 0)
            {
		...
		/// <summary>
        /// DELETE api/DimIndCate/Delete/?ids="1,2,3,4"
		/// </summary>
        public int  DeleteIds(string  ids)
        {
			try	{
				DimIndCateDAO.Del(ids);
			}
			 catch (Exception ex)
            {
                //批量删除是出错
        ...
        public  JsonResult TreeAll()
        {
            ...
            IList<TbDimIndCate> items = DimIndCateDAO.GetByPageDataBase(0, int.MaxValue, "ILevel ASC,IParentID ASC,Id ASC", "1=1", out totalCount);//按ILevel升级，再按ParentID排序
            ...
            for (int i = 0; i < items.Count; i++)
            {
                ...
                myNode.name = items[i].SCateName;
				myNode.open = false;

                myNode.id = items[i].Id;
				myNode.pId = (int)items[i].IParentID;
                myNode.tip = items[i].SCateName;
		...
		/// <summary>
        /// DELETE api/DimIndCate/TreeDel?id=5
		/// </summary>
       ...
       [HttpPost]
        ...
        public int DelTreeNode(int id)
        {
            ...
            try
            {
                TbDimIndCate parent = DimIndCateDAO.GetByID(id);
        ...
        public JsonResult DimLibIndicatorTree(int libId) {

            IList<TbDimIndCate> items = DimIndCateDAO.GetByPageDataBase(0, int.MaxValue, "ILevel ASC,IParentID ASC,Id ASC", "ILibID="+libId, out totalCount);//按ILevel升级，再按ParentID排序
        ...
        public JsonResult DimLibIndicatorTreClose(int libId)
        {
