...
namespace HyCtbuEco.Admin
{
	...
	#region 表生成代码
        ...
        /// <param name="count">取得记录数</param>
        /// <returns></returns>
        public IList<TbSysPagePower> GetByPage(string whereClause, int start, int limit)
        {

            IList<TbSysPagePower> tmplist;
            string Sqlstring = SQLHelper.GenPageSQL("tb_Sys_PagePower", whereClause, start, limit, "", "");
            using (var connection = DataBase.GetOpenConnection())
            {
        ...
        public bool  Del(string ids)
        {
           ...
           IList<TbSysPagePower> tmpList=GetByPage("id in ("+ids+")",0,int.MaxValue);
			return  Del(tmpList);
        ...
        /// <returns></returns>
        public int Insert(TbSysPagePower item)
        {
            int id = 0;
            using (var connection = DataBase.GetOpenConnection())
            {
                id = (int)connection.Insert<TbSysPagePower>(item);
            }
            return id;
        }
...
//	END
        ...
        public bool DelTree(int id)
        {
			...
			IList< TbSysPagePower > items = GetByPage("ID in (select id from [fn_SysPagePowerGetChildren](" + id.ToString() + ") ",0,int.MaxValue);
